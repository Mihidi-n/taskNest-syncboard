...
export default function TaskDetailModal({ task, canEdit, onClose, onSave, onDelete }) {
  ...
  const handleAddLabel = () => {
    ...
    setLabels((current) => [...current, { name: trimmedName, color: newLabelColor }])
    setNewLabelName('')
  }
  ...
  const handleLabelChange = (index, field, value) => {
    if (!canEdit) return
    setLabels((current) =>
      current.map((label, idx) => (idx === index ? { ...label, [field]: value } : label))
    )
  }
  ...
  return (
    ...
    >
      <div className="task-detail-modal__content" onClick={(e) => e.stopPropagation()}>
        ...
        {!canEdit && (
          <p className="task-detail-modal__note">You have view-only access to this board.</p>
        )}
        ...
        <section className="task-detail-modal__section">
          <div className="task-detail-modal__section-header">
            <div>
              <h3>Labels</h3>
              <p className="task-detail-modal__help">Create or edit labels for this task.</p>
            </div>
        ...
        <footer className="task-detail-modal__actions">
          ...
          <div className="task-detail-modal__actions-right">
            <button type="button" className="task-detail-modal__cancel" onClick={handleClose}>
              {canEdit ? 'Cancel' : 'Close'}
            </button>
            {canEdit && (
              <button type="button" className="task-detail-modal__save" onClick={handleSave}>
                Save
              </button>
            )}
          </div>
